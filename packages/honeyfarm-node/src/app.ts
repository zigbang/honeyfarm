...
export class honey {
	...
	private readonly appiumDefaultPort = Number(process.env.START_PORT) || 4724
	...
	private async updateDeviceStatus() {
		...
		const unregisteredDevices = await Promise.all(
			Object.entries(this.resources).map(async ([serial, device]) => {
				const exist = await API.getDevice(device.port, device.platform, device.version, serial)
				if (!exist) {
					return serial
				} else {
					return undefined
	...
	private async addDeviceToLocal(serial: string, platform: "android" | "ios") {
		...
		try {
			...
			if (platform === "ios") {
				...
				if (process.env.USE_CRHOMIUM) await this.getChromiumDriver(serial)
			}
	...
	private startAppiumServer(serial: string, port: string, wdaPort: string) {
		try {
			...
			const appiumOptions = ["-p", `${port}`, "--webdriveragent-port", `${wdaPort}`]

			if (!process.env.USE_CRHOMIUM) appiumOptions.push("--relaxed-security")
	...
	private async addDeviceToServer(serial: string) {
		...
		try {
			...
			await API.registerDeviceStatus(device.port, device.platform, device.version, serial, device.name, device.wdaPort, device.mjpegServerPort, device.type)
	...
	private async removeDevice(serial: string) {
		try {
			...
			await API.deregisterDeviceStatus(port)
	...
	private async getChromiumDriver(serial: string) {
		if(!process.env.USE_CRHOMIUM) return
